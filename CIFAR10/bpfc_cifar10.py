...
TEST_BATCH_SIZE  = 100
BASE_LR          = 1e-1
MODEL_PREFIX     = 'models/cifar_' + EXP_NAME + '_'
...
for epoch in range(epochs):
    ...
    for data, target in train_loader:
        ...
        qnoise = torch.Tensor(data.size()).uniform_(-1,1).cuda()*qnoise_scale
        qdata = qdata + qnoise
...
##################################### PGD, steps=[7,20,50,100,500,1000] #############################################
log_file = open(EVAL_LOG_NAME,'a')
msg = '##################### PGD: steps=[7,20,50,100,500,1000],eps_iter=2/255 ####################\n'
log_file.write(msg)
log_file.close()
all_steps = [7,20,50,100,500,1000] 
