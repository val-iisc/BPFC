...
BASE_LR          = 1e-2
MODEL_PREFIX     = 'models/fmnist_'+EXP_NAME+'_epoch_'
...
##################################### Load Network ################################################
execfile('FMNIST_Network.py')
model = FMNIST_Network()
model.cuda()
model.train()
...
val_set    = torchvision.datasets.FashionMNIST(root='./data', train=True , download=True, transform=transform)
...
print('Fashion-MNIST dataloader: Done')
...
msg = '##################### iter.FGSM: steps=40,eps=0.1 ####################\n'
log_file.write(msg)
log_file.close()
...
log_file.write(msg)
log_file.close()
for eps in np.arange(0,0.101,0.02):
    accuracy = 0
...
log_file.write(msg)
log_file.close()
for eps in np.arange(0.02,0.101,0.02):
    accuracy = 0
    for data, target in test_loader:
...
log_file = open(EVAL_LOG_NAME,'a')
msg = '############## PGD: steps=[20,40,100,500,1000], eps=0.1, eps_iter=0.01 ###############\n'
log_file.write(msg)
log_file.close()
